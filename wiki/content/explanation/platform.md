...
| 24 | unreleased | goal status splits into two axes: `goals` gains `lifecycle_status` and retires `status`; `goal_progress` gains `lifecycle_status` and `achievement_status`. Old lines keep validating and map forward - `paused` to `on_hold`, `abandoned` to `cancelled`, `achieved` splits into lifecycle `completed` plus achievement `achieved`. `status` keeps its column on `goal_progress` with the same value, so a consumer reading the old name is unaffected |
