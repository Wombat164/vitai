...
| 26 | unreleased | `best_efforts`: the fastest 1k, 5k, 10k, half and full of every stored track, one row per (track, distance). Read `basis` before quoting a time - `device` is measured against the watch's own distance, `derived` against the engine's haversine sum. `seconds` is elapsed, so a stop inside the window counts |
