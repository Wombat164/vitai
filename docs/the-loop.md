...
104. Is an unassessed symptom trending (the stop-programming tripwire)? - tripwires + [G11 links symptom lines to the eventual visit]
...
176. I lost 2kg but my average km/h didn't improve - did the model fail? - no: an expectation-vs-actual signal (G21). Candidates: water/glycogen not fat lost, muscle loss, deficit fatigue, near the body-fat floor, or noise over too short a window - enumerated, never "the formula failed". [G22 x G21]
...
219. I forgot my phone so I could not badge into the gym - is that the app's problem? - yes: an access credential is a HARD GATE, a feasibility precondition like opening hours, not a nicety. Kit requirements (towel, bag, entry QR, shower stuff when finishing away from home) are modelled per activity x place, and what the place already affords (showers, towel hire) changes the packing list. [G53 x G48]
220. Why can't I run home from the supermarket? - because you are carrying 10 kg of shopping. CARRY LOAD CONSTRAINS LOCOMOTION MODE: carried mass and bulk decide which modes the next leg can use (loaded = walk; a light pack = a slower run; hands full = neither). Carry state is a planning variable, not a detail. [G53]
221. Tell me about the kit AFTER I have driven 45 minutes in civilian clothes and it is useless - so when? - at the last moment you can still act: before you leave home in the morning, for the session tonight. PREPARATION LEAD TIME is the point; a pre-departure checklist is one of the few nudges that earns its interrupt because it is high-value, time-critical and cheap to act on. [G53 x G39]
222. Gym, then shopping, then home - why does the app have to see that as one thing? - because each leg MUTATES the state the next leg depends on: shopping adds carry-load which downgrades the run home to a walk; the session leaves you sweaty; a one-way route consumes its outbound transport. Legs cannot be validated independently - the planner checks the whole chain and says where it breaks. [G54]
223. I want to run one-way from a town up the coast back home - can the app just plan it? - it can plan it, but the outbound leg must be SECURED, not merely chosen: a bus with a real timetable, a taxi to book, or a lift from a family member (a dependency on another person). Until that is arranged it is a proposal, not a schedule, and the coach says so. It never books on your behalf. [G54 x G51]
224. Why does the app care about my shoes, my watch strap, my new racket? - because owned gear GATES what can be proposed (no racket, no tennis), and NEW gear is a signal a sport may be entering your life - worth a question, not silence. Kit in the wash or a dead strap battery blocks a session as hard as not owning the thing. [G55]
225. Can it tell me when my running shoes are done? - yes, and deterministically: sum the run distance logged on that pair since purchase and compare to the replacement threshold (~600-800 km). Worn shoes are an injury-risk factor, so this is a safety-adjacent derived metric in the number path - not a shopping nudge. [G55 x P4]
226. I have shoes at home but I'm at the holiday flat - does the app know? - it should: gear has a LOCATION, and single-instance items do not teleport. Shoes at home are not shoes here, a watch on the charger at home is unavailable, and if a household member has the one item it is contended like any shared asset. [G55 x G48 x G52]
227. I want to ramp fitness AND hold a deficit AND not get injured - which do I sacrifice? - none, if you stop paying for calories with the expensive currency. The deficit is bought with TIME ON FEET (walking: no ramp cost, no injury price); fitness is bought with RUNNING VOLUME (scarce, injury-priced). Cap the running at what your ramp affords, then buy the remaining burn with walking. [G56]
228. So a long run "for the burn" is a mistake? - usually yes: it spends scarce ramp budget on something cheap movement buys for free. A walk-run-walk sandwich delivered the SAME deficit at ~45% less running load, and the walks doubled as warm-up and cool-down - strictly better, not a compromise. [G56]
229. If the guardrail says no, do I just do nothing? - never: a rejection must ship a COUNTER-OFFER. "Doing something beats a rest day" is a real adherence fact, not a weakness to correct, so the coach re-routes to the same outcome through a cheaper currency instead of declining. [G56 x P7]
230. Won't scaling back kill my visible stats? - less than it feels: walks log on the tracker too, and the stat that COMPOUNDS is weekly consistency - a 5k you repeat three times beats a 9k that costs you two weeks. The vanity number that will actually move is your easy pace, and that is built by easy running, not hammering. [G56, motivational honesty]
231. Why does the app only track what IT thinks matters? - it shouldn't, and that was the biggest hole we found. Asked what they actually wanted counted, eight people named nights without the 3am vending machine, belt holes against a kept wedding-day belt, '12 of 14' stairs, times they said yes instead of 'you go on'. Almost none of it fit the schema. You must be able to declare your own metric and have the engine treat it as first-class. [G78]
232. Some of what I count deliberately ignores how much - is that allowed? - not only allowed, required. 'Five useless minutes counts the same as the full session, because the not-showing-up is the actual enemy.' 'Whether we did it - not the distance, not the calories.' Quantifying those would destroy exactly what they measure. [G79]
233. Half of what I care about involves someone else - can the app hold that? - it must: six of eight athletes named a relational metric unprompted (did we eat together, did we walk together, did the kids come and watch AND WHICH ONE). The dyad is the subject, not the athlete with a person attached. [G80]
234. There are things I count that I don't want anyone to see, and one I don't want recorded at all. - both are honoured: per-metric visibility including hidden-from-the-coach, and a refusal to capture. A tally of bad shifts can be known to exist and deliberately not held - 'some of it isn't for tracking'. A system that captures everything you mention is one you stop talking to. [G81]
235. There are months of my life I can't explain and don't want to. - the gap is DATA, not missing data, and often where the real story is. The engine never demands a reason, treats a stated reason as a claim (one athlete's log blames an Achilles for months his father's stroke actually caused), and knows the most informative period may contain no data at all. [G82]
236. In five years, what would I be sad to have lost? - not a number. None of eight athletes named one: 'that I still had a me outside the ward', 'who was standing at the finish line, not the time on the clock', 'if I've kept the weight off but I'm still the mum who says you go on, I'd count that as having lost'. Targets need a layer above them: the thing they are FOR. [G83]
237. Why does the app know 'gym_a' but not 'cycling'? - because the vocabularies were written from one athlete's examples rather than designed as taxonomies. `gym_a`/`gym_b` are literally one person's programme labels shipped in a public engine, while cycling, swimming, rowing and every team sport fall into `other`. One axis per vocabulary, grounded in real prior art, in a registry rather than in code. [G85]
238. Why couldn't it record 'no loaded lumbar flexion'? - because `restricts` mixes four axes (a scope quantifier, a setting, a loading modality, anatomical regions, specific activities) and none of them expresses a movement pattern under load. Post-coordinating it - pattern + region + load, reusing the body-site registry - expresses both that gate and 'no loaded hip work, squats still fine', neither of which the flat list could. [G85 x G59]
239. Who gets to add a value to a vocabulary? - today only a developer, which is exactly why the lists only ever contain what the developer had seen. Moving vocabularies into `semantics/*.toml` registries makes them athlete-extensible, which is the same fix G78 needs for athlete-defined metrics. One change, two problems. [G85 x G78]
240. My daughter's wedding is in 14 months and my 10k is in June - are those goals? - no, they are EVENTS: dated real-world fixtures that cannot be moved and that training is planned backwards from. The engine only has derived milestones (25/50/75/100% of a target), which is a different thing wearing the same word. [G86]
241. Is 'weigh 73 by Q1 2027' the same kind of deadline as a race date? - no, and the difference matters twice: a hard date forces a taper and raises urgency as it nears, while a soft one is a direction of travel you may move at no cost to anyone. Without the distinction the churn logic flags moving your own invented date as goalpost-moving, which is a trust-destroying false accusation. [G86 x G20]
242. What about a goal nothing can measure - 'I want to enjoy running again'? - it must still be holdable. That is not G19's external (another app measures it); nothing measures this, ever, and your word is the only evidence there will be. The engine tracks it, surfaces it and asks about it, and never issues a verdict on it - which matters because what people say they would be sad to lose in five years is almost always exactly this kind of goal. [G86 x G83]
243. If I mask my home by a random point within 100 m, am I safe? - not quite, on two counts. A FRESH random draw each export is averaged away: N exports with independent noise converge on your true door as sqrt(N), so the offset must be deterministic per place and identical every time. And 100 m is about a block - the redaction literature puts street-block scale at 150-450 m. [G87]
244. Surely blurring the whole route is the safe option? - unnecessary and costly. Re-identification concentrates at the ENDPOINTS (a known home/work pair is unique at census-block granularity); the interior carries far less risk and needs only 10-50 m precision for route matching. Mask the first and last 100-200 m heavily and leave the middle alone. [G87]
245. My holiday flat is temporary - does it matter? - more, not less. It is a home for that fortnight, and publishing it also announces that your real home is empty. Any place you sleep gets anchor treatment. [G87]
246. If every coordinate is masked, is my route data safe to share? - no: HABIT is a separate leak. 'Runs 21:30 Tue/Thu/Sat from the same anchor' is a schedule that says when a house is empty, and it egresses even when the geometry is perfectly masked. Timing coarsens on the same toggle. [G87]
247. Why hand-roll a route-similarity metric when Strava has solved this? - you should not, and doing so is the same mistake as inventing a vocabulary from your own examples. A 38 m grid-overlap ratio invented on the spot has no published threshold, cannot be tuned, and cannot be told apart from a bug; the real problem has named solutions (Frechet, DTW, LCSS, map-matching). Find the algorithm and the maintained library first - and ship it as versioned code with tests and a CLI+API surface, because a scratchpad script is not an implementation and its numbers are not evidence. [G85 extended to algorithms]
248. If my home is masked before it leaves the device, is that enough? - no, and unlabelled masking is worse than none. A receiver that is not told will compute on the fake anchor as though it were exact: distance from a masked start, "you began somewhere unusual today", a habit pattern from coarsened times. The mask must announce itself, state its error band, and say what may not be derived from it. [G88]
249. Why is a GOOD mask more dangerous than a sloppy one? - because it is believable. The pseudo-anchor is deterministic on purpose, so repeated exports cannot be averaged back to your door - and that same self-consistency makes it indistinguishable from a real reading. A noisy mask wobbles suspiciously; a stable one is perfectly plausible and silently wrong. The property that buys the privacy invites the false inference. [G88 x G87]
250. What does a masked value actually look like on the wire? - it is an observation with a band, not a coordinate: a 300 m keyed offset IS a +/-300 m reading and must present as one, reusing the same uncertainty-interval mechanism as a jittery scale. Anything derived from it either carries the band forward or refuses to compute - a precise-looking number from an imprecise input is confidence laundering arriving through the privacy layer. [G88 x G37 x P3]
...
| G28 | **Safety escalation** - DETERMINISTIC severity->action (not LLM prose): cardiac/red-flag symptom class, absolute-danger thresholds, a low-energy-availability composite, a fast path bypassing the weekly cadence, the written never-shame carve-out. | CRITICAL | P4 / P7 |
...
| G53 | **Kit, attire, access credentials & carry-load** (operator, 2026-07-29) - the logistics layer that silently kills plans. Kit requirements per activity x place (towel+bag for the gym; shower kit/fresh clothes when finishing away from home - modulated by what the place affords, G48). ACCESS CREDENTIALS ARE HARD GATES (no phone -> no entry QR -> no session; a feasibility precondition, not a convenience). ATTIRE STATE (civilian vs running kit; changing needs kit + a place). CARRY LOAD CONSTRAINS LOCOMOTION MODE (10 kg of shopping = walk not run; a loaded pack slows a run). PREPARATION LEAD TIME: surface kit at the last moment the athlete can still act (before leaving home), never on arrival - a pre-departure checklist earns its interrupt. | MEDIUM | P8 / G48 / G52 |
| G54 | **Trip chaining & leg-state propagation** (operator, 2026-07-29; extends G44/G51) - a journey is a SEQUENCE OF LEGS and each leg MUTATES the state the next depends on, so legs cannot be validated independently (shop after the gym -> carry-load -> the leg home downgrades run->walk; a session leaves you sweaty; a one-way route consumes the outbound transport). Validate the WHOLE CHAIN and report where it breaks. TRANSPORT LEGS MAY NEED SECURING not just choosing: a bus with a real timetable, a taxi to book, a lift from a family member (a dependency on another PERSON, G51). An unsecured leg makes it a proposal, not a schedule, and the coach says so. Never books autonomously. | MEDIUM | G44 / G51 |
| G55 | **Owned gear inventory & consumable lifecycle** (operator, 2026-07-29; extends G53/G48/G52) - what is OWNED, not carried: shoes, watch, HR strap, hat, sunglasses, apparel, sport-specific kit (stick, racket). (1) Gear GATES activities - propose only what the athlete is equipped for; NEW gear signals a sport entering the plan, worth asking about. (2) Consumables have a deterministically trackable LIFECYCLE - shoe mileage vs a ~600-800 km replacement threshold is a derived number-path metric and injury-risk-adjacent, not a shopping reminder (same shape: strap batteries, worn kit, expiring memberships). (3) Gear has a LOCATION and is often single-instance - shoes at home are not shoes at the holiday flat; contended like any shared asset. Condition (dead battery, kit in the wash) gates use as hard as absence. | MEDIUM | G53 / G48 / G52 |
| G56 | **Goal currencies & equivalent-outcome substitution** (operator, 2026-07-29) - goals are bought in different CURRENCIES at different risk prices: the deficit goal with TIME ON FEET (walking, nearly free), the running-fitness goal with RUNNING VOLUME (scarce, injury-priced). The characteristic error is paying for calories with the expensive currency (a long run "for the burn"). When a guardrail rejects the athlete's plan the coach SUBSTITUTES rather than declines - cap the risk-priced part at what the ramp affords, buy the rest with the free one (walk-run-walk: identical deficit, ~45% less running load, walks double as warm-up/cool-down). A rejection always ships a COUNTER-OFFER ("doing something beats a rest day" is an adherence fact, not a weakness); equivalence computed from the athlete's OWN kcal/min rates, never textbook METs; name honestly what is lost/gained incl. the motivational stat. | HIGH | P7 / G18 / G45 |
| G57 | **Life-stage & physiological states** (persona validation, 2026-07-29) - breastfeeding/pregnancy/postpartum/growth/menopause/illness change energy REQUIREMENTS and SAFETY BOUNDS; a deficit can be CONTRAINDICATED, not merely aggressive. A different axis from G34 situational context. Without it the engine helps an athlete run a dangerous deficit. | CRITICAL | P3 / G28 / G34 |
| G58 | **Goal safety & feasibility at declaration** (persona validation) - an unsafe/impossible goal (12 kg in 6 weeks) is currently stored and tracked faithfully, reporting the athlete BEHIND while they eat less to catch up. Declaration needs a gate (rate bounds, life-stage contraindication, deadline sanity) whose output is a NEGOTIATION, never silent compliance nor a bare rejection. | CRITICAL | P1 / G6 / G28 |
| G59 | **Red-flag capture from PROSE + resolution scope** (persona validation) - red flags arrive as downplayed asides ('the odd twinge but it's nothing' = EXERTIONAL chest pain; 'I nearly blacked out'), NEVER as data; G28 reads structured entries and would see nothing. Fix preserving P4: LLM recognises+classifies symptom language into a claim (G43), engine maps severity->action with hardcoded text. Corollary: a prior negative workup must NOT suppress a new/changed symptom - resolution has scope + expiry. | CRITICAL | P4 / G28 / G43 |
| G60 | **Cadence unit is not the calendar week** (persona validation) - verdicts/rollup/streaks/periods bucket by Monday-anchored week; for rotating shift workers that unit is fiction. Needs configurable cadence: calendar week | rolling N days | user-defined cycle (shift block). Large population, not an edge case. | HIGH | P2 / G30 |
| G61 | **Subjective day anchor** (persona validation) - G30 fixed tz/DST but kept a midnight day. A night worker's shift straddles midnight and 'last night's sleep' lands in a calendar afternoon; the boundary must follow SLEEP (wake-to-wake). Naive midnight bucketing splits every night shift in two. | HIGH | P2 / G30 |
| G62 | **Goal kinds + proxy indicators** (persona validation) - 'one pull-up' is target=1 with progress 0,0,0,...,1: monotonic/guarded is meaningless, milestones generate nothing. Needs goal `kind` = quantity | skill (binary) | maintenance, and for skill goals PROXY/leading indicators (hang time, lowering tempo, assistance load) carrying the visible progress the goal cannot. Generalises past fitness - P8 genericity actually tested. | HIGH | G6 / G18 / P8 |
| G63 | **Re-entry contract & sanctioned pause** (persona validation) - the dominant adherence failure is RE-ENTRY, not training ('I miss two, feel I've fallen off, three weeks pass, I feel stupid going back, so I don't'). Needs a re-entry contract (resume at SAME load, never asked where you've been, a lapse is not a broken streak) AND a sanctioned pause as a declarable state with a minimum-viable dose - because the ABSENCE of shame is not the PRESENCE of permission. Plus a pre-authorised integrity check on the pause. | HIGH | P7 / G8 |
| G64 | **Low-data / deviceless mode + plain language** (persona validation) - 2 of 3 personas had no wearable and no intention of getting one; one tracks nothing and distrusts apps. Resolution/kcal_out/HR caps/RHR baselines all assume device data; P8's 'minimum day is one number' was never tested. Needs a qualitative mode (did it happen, how did it feel) plus plain-language translation of the athlete's own clinical numbers. | HIGH | P8 / P3 |
| G65 | **Goal contention & deliberate deprioritisation** (persona validation) - goals COMPETE for one finite budget of time/energy/attention (a certification exam eating the same days off, and mattering more). 'This goal outranks that until November' must be declarable and change what the coach asks for. | MEDIUM | G18 / G6 |
| G66 | **Occupational & incidental activity** (persona validation) - 'I do miles a day but never measured owt': 8 h/day on his feet is likely the largest energy term and is unmodelled. The engine assumes sessions + device steps; for manual workers occupational load dominates both. | MEDIUM | P1 / G22 |
| G67 | **Off-limits domains & deferred levers** (persona validation) - G33 suppresses a METRIC; this is a whole intervention DOMAIN declared untouchable (a partner's cooking) and separately a lever the athlete PARKS. Off-limits = respected silently and worked around; deferred = named honestly ONCE without moralising, then left alone. Nagging either loses the athlete. | MEDIUM | P7 / G33 |
| G68 | **Safety must not be opt-in; defaults fire unconfigured** (engine run on persona data, 2026-07-29) - a nursing mother on ~1200 kcal losing ~1 kg/wk with a near-syncope produced `tripwires: none`, because rate verdicts need configured phase targets and tripwires need configured thresholds, and she had configured nothing. The safety apparatus is INERT BY DEFAULT - backwards for the population most at risk. Absolute-danger rules must fire from defaults; config may tighten the net, never create it. | CRITICAL | P4 / G28 |
| G69 | **Signed quantities must be labelled, not bare** (engine run) - rollup showed `Rate: +1.10 kg/week` for an athlete who LOST 1.5 kg (convention is older-minus-newer, so + means losing). For a scale-anxious under-eater, reading + as a gain is the input most likely to make her cut further. State direction in WORDS, never a bare sign. | HIGH | P7 / G34 |
| G70 | **Resistance training is unmodelled** (engine run) - every `sessions` field is an ENDURANCE field; there is nowhere to record a set, a rep or a load, so a gym session is a duration plus a note. Blocks strength goals entirely, progressive overload as a signal, and the G36 Tier-2 recomposition proxy that depends on it. Also `setting` has no `gym` value. A large hole for a project claiming genericity. | HIGH | P8 / artifact-1 |
| G71 | **Quantities stored as booleans and scalars** (engine run) - `alcohol` is boolean (5 pints and 1 can both `true`, hiding ~1400 kcal/weekend); `sleep_h` is a scalar (fragmentation and timing are the whole story for a broken-sleep mother and a night-shift nurse, and neither survives). Where a quantity or structure exists, store it. | MEDIUM | P1 / P3 |
| G72 | **Medication as a first-class modifier** (sweep 2, 2026-07-29) - a GLP-1 agonist makes sub-800 kcal days INVOLUNTARY (not restriction, not non-compliance), makes rapid loss EXPECTED (so a naive rate tripwire misfires), and shifts the real risk to lean-mass loss from rapid loss + low protein + no resistance work. Beta blockers invalidate HR zones; insulin/sulfonylureas create exercise hypoglycaemia risk. Voluntary and involuntary under-eating need OPPOSITE interventions and nothing distinguishes them. | CRITICAL | P3 / G57 / G28 |
| G73 | **A CLINICAL HOLD tier above 'flag'** (sweep 2) - recurrent bone stress injury + amenorrhea + energy availability far below threshold is not a tripwire to list beside the week's rate line. Correct behaviour: SUSPEND algorithmic progression, BLOCK the coach from giving training advice on the case. G28 escalates a message; this escalates by disabling the product's own advice. | CRITICAL | P4 / P7 / G28 |
| G74 | **The athlete's benign self-explanation is not evidence** (sweep 2) - FOUR of eight personas supplied an innocent cause for a concerning sign: 'it's the reflux' (exertional chest pain), 'that's what happens when you train hard' (5 months amenorrhea), 'my legs aren't used to the lighter me' (weakness during rapid GLP-1 loss), 'I stood up too fast' (near-syncope while under-eating). The attribution is a CLAIM (P1) and must never suppress the pattern it explains away. | HIGH | P1 / P3 / G59 |
| G75 | **Subjective and objective measures are different quantities** (sweep 2) - device says 6.1 h, athlete says 'felt like 5', systematically. Not competing claims to resolve (G15) - both true, and the DIVERGENCE is the signal (fragmented sleep at normal duration is the perimenopause presentation). Resolution must not collapse perceived into measured. | MEDIUM | P1 / P3 |
| G76 | **Logging bias is not logging coverage** (sweep 2) - 'I log breakfast and lunch because those already look fine': logged days are systematically biased LOW, not merely incomplete. `coverage: partial` says something is missing; it cannot say what is PRESENT is unrepresentative. A biased sample read as complete produces confidently wrong energy arithmetic. | MEDIUM | P1 / G26 |
| G77 | **Capacity-limited progression** (sweep 2) - with knee osteoarthritis the limiter is PAIN not fitness; 15 min of walking is a hard ceiling no ramp rule progresses through, and the goal is approached by changing the constraint (load, surface, strength, clinical input), not by adding volume. Progression logic assumes the limiter is conditioning. | MEDIUM | G6 / G62 |
| G78 | **Athlete-defined metrics: the schema is a developer's ontology** (sweep 3, 2026-07-29) - CRITICAL and the largest finding of the programme. Sweeps 1-2 only asked for metrics the DEVELOPER chose, so could only find gaps in what was already imagined. Asked what THEY wanted counted, eight athletes named almost nothing the schema holds - many counted privately for years. The athlete must be able to DECLARE a metric (name, kind, unit, scale, cadence) and have the engine count/streak/trend/celebrate it without a developer. | CRITICAL | P8 / P5 |
| G79 | **Metric KINDS beyond measurement** (sweep 3) - counters, days-since-with-reset, reset-on-fail streaks, fractions of a named obstacle ('12 of 14 stairs'), pass/fail against a personal landmark, occupational capacity, affective quality ('did it feel EASY - not RPE'), deliberate-omission counts (skipped guilt-free, no makeup session). MAGNITUDE IS OFTEN EXCLUDED ON PURPOSE - 'five useless minutes counts the same as the full session'. Quantifying destroys what they measure. | CRITICAL | G78 / G62 |
| G80 | **Relational metrics: the dyad is the subject** (sweep 3) - SIX of eight named one unprompted: did we sit down together; phone-free conversations; days since a proper meal together; whether we walked together ('not the distance - whether we did it'); whether the kids came to watch AND WHICH ONE. The engine is single-athlete throughout. | HIGH | P8 / G51 |
| G81 | **Per-metric privacy + the refusal to capture** (sweep 3) - metrics disclosed WITH privacy conditions ('private, not something that shows up on a screen anyone else sees'; 'I would never say this to my old coach'). Needs per-metric visibility incl. HIDDEN-FROM-COACH. Harder case: a quantity kept, not recorded, and not to be recorded ('some of it isn't for tracking'). The model must know a thing exists and deliberately not hold it. | HIGH | G32 / G33 |
| G82 | **The unaccounted period** (sweep 3) - EIGHT of eight independently described months that resist any form, and in every case that is where the story lives. (a) A gap is DATA, not missing data. (b) The recorded reason may be a self-protective fiction - one log blames an Achilles for a drop actually caused by his father's stroke. (c) The most valuable period may hold NO data (ten weeks off eating ad-lib = the natural experiment showing recovery, 'a hole in the spreadsheet'). Never demand a reason for a gap. | HIGH | P1 / P2 / G34 |
| G83 | **What they'd be sad to lose is never a number** (sweep 3) - asked about five years out, NOT ONE of eight named a metric, target or weight; all named an identity or a relationship ('that I still had a me outside the ward'; 'if I've kept the weight off but I'm still the mum who says you go on, I'd count that as having lost'). The goal model is target-based all the way down; it needs a layer ABOVE targets - the thing the targets are FOR. | HIGH | P7 / G6 |
| G84 | **Conflict shapes G56 cannot express** (sweep 3) - (a) COST-BEFORE-BENEFIT and the trough is where people quit ('cutting the wine costs me before it pays me anything back, and that gap is exactly where I've quit before'); (b) NEGATIVE RETURN on effort (pushing flares the knee -> two lost days -> less total activity; ramp logic assumes monotonic returns); (c) THE INTERVENTION DESTROYS THE MECHANISM ('eating deliberately means it stops feeling like the drug is doing it for me'). Plus: a conflict may be correctly left unresolved. | HIGH | G56 / G65 |
| G85 | **Genericise before encoding: one axis per vocabulary, registry not code** (operator, 2026-07-30) - CRITICAL process gap that produced several others. Closed vocabularies were written from ONE athlete's examples: `SESSION_TYPES` ships `gym_a`/`gym_b` (personal programme labels) in a public engine; `ACTIVITY_CLASSES` mixes scope+setting+loading+region+activity and could not express two real clinical gates; `CONTEXT_MODES`, `SEVERITIES`, `SETTINGS`, `WEATHERS`, `MEASUREMENT_KINDS` all mix axes or bake in units. RULE: one axis per vocabulary, POST-coordinate (the body_sites pattern - the only one that got a prior-art sweep and the only one without the defect), ground in prior art (Compendium, ICF, FHIR value sets), and live in `semantics/*.toml` as a registry. A vocabulary in CODE can only be extended by a developer, so it only ever holds what the developer had seen - which is also why G78 needs the same fix. Issue #18. | CRITICAL | P5 / P8 |
| G86 | **Events, deadline hardness, attested-only goals** (operator, 2026-07-30) - (a) an EVENT (wedding, race, competition weigh-in) is a dated immovable real-world fixture that periodisation is built backwards from, NOT a derived progress fraction; only the derived kind exists. (b) A deadline is HARD (immovable, externally owned) or SOFT (self-imposed, movable at no cost) - and G20's churn logic MISFIRES without it, flagging a legitimately-moved soft date as goalpost-moving. (c) ATTESTED-ONLY goals ('enjoy running again') have no metric and never can - distinct from G19 external; the schema requires a non-empty metric so they have no home, yet G83 found this is what athletes actually value. (d) Goals overlap and couple; the RELATION is unmodelled. | HIGH | G6 / G20 / G62 |
| G87 | **Egress masking for location and habit** (operator, 2026-07-30) - risk is EGRESS not storage, governed by one toggle. (a) ENDPOINTS are the asset: Golle+Partridge 2009 put the anonymity set for a known home/work pair at census-block scale at ONE; home detection clusters stay points at trip start/end. Mask endpoints heavily, keep the interior (route matching needs only 10-50 m). (b) A STABLE pseudo-anchor, never a fresh random draw - independent noise per export averages back to the true centre as sqrt(N). Deterministic keyed offset per place. (c) Radius must exceed building scale (H3 res8-9, ~150-450 m); 100 m is likely too tight. (d) k-anonymity cloaking does NOT apply - a single-user record has no population to hide in, which is the 2018 Strava heatmap failure. Temporary anchors (a holiday flat) count and also announce the real home is empty. HABIT is its own leak: a recurring schedule egresses even with every coordinate masked. The toggle is a hard boundary, not a rendering preference. | HIGH | P8 / G32 / G35 |
| G88 | **Degraded data must be self-describing at every boundary** (operator, 2026-07-30) - if a value is masked/coarsened/rounded/imputed/defaulted/sampled on the way out and the receiver is not TOLD, it is computed on as exact and yields confidently wrong inferences; the masking then does more harm than none. THE TRAP IS SPECIFIC TO GOOD MASKING: a stable pseudo-anchor is deterministic precisely so exports cannot be averaged back to the truth, and that self-consistency makes it indistinguishable from a real observation - noisy masking wobbles suspiciously, deterministic masking is perfectly believable and silently wrong. A degraded value carries what was done, its ERROR BAND (reusing G37 - a 300 m offset IS a +/-300 m observation), and what may NOT be derived from it. Derivations over degraded inputs propagate the band or refuse; a precise number from an imprecise input is P3 confidence-laundering arriving via the privacy layer. A CONTRACT at the boundary, not a hope about the consumer. Generalised past masking to any lossy transform. | HIGH | P3 / G37 / P4 |
