...
| vocabulary | one new verdict word `undecidable` (interval spans the whole band). Closed-vocab addition = read-model contract bump (CONTRACT_VERSION in db.py). `no_data` keeps meaning "nothing to compute from" (unwitnessed); `undecidable` means "computed, and the instrument cannot support a verdict" - the I5-style distinction between an empty record and a refusing one. |
...
| imported numeric error parameters | stream 2 reversal: borrowed figures never enter arithmetic (01-schema s4) | nothing; own replicates and overlap windows are the routes |
| condition-scoped capability matching for sessions | observations do not record condition (tree cover, treadmill flag partially exists as `setting`) | wire `setting` -> `condition` once vocabularies align |
