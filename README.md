...
Contracts 9 to 16 are missing from this table rather than from the record: the full per-contract history lives beside `CONTRACT_VERSION` in `src/vitai/db.py`, and this summary stopped being maintained around contract 8. Read `db.py` for anything not listed here.
