...
| `data/overlaps.jsonl` | the counted window two instruments both covered - the evidence a comparability row cites | `dataset`, `field`, `origin_a`, `origin_b`, `paired_days`, `from_date` |
...
| 48 | unreleased | `crossings.kind` gains `band`: #370's third milestone kind, a population-reference ratio (BMI) crossing a boundary | **No column moved** - `CROSSING_KEYS` is unchanged, the same seven columns contract 47 shipped. What moves is the closed vocabulary `kind` may hold, which is a consumer-visible change on its own: code matching `kind in ("round_number", "personal_first")` silently drops every `band` row rather than erroring on it. **The engine may compute the ratio and state the boundary as a boundary; it may never name the band** - decided by the operator on the issue. A `band` row's `value` is the numeric boundary crossed, never a word, the same way a `round_number` row's `value` is the rung and not a sentence about it - nothing on this row has a field that could hold a category name. Boundaries are ADOPTED, NOT INVENTED (G85): fetched and cited in `crossings.py`, and a finer subdivision common in secondary sources is left out because it could not be confirmed against a source this change could actually read. Height is EFFECTIVE-DATED, reusing `policy._in_force`'s own machinery (`policy.height_on`) rather than a second one - a weight reading with no height yet in force mints no ratio, never one back-filled from a later height |
| 49 | unreleased | `questions.kind` gains `outage` and `false_zero`: #398's fourth and fifth kinds, an instrument that stopped and a zero that should have been an absence | **No field moved on a question row** - what widens is the closed vocabulary `kind` may hold, and a client matching `kind in ("precondition", "clearance", "waking")` silently drops both new kinds rather than erroring. **Two kinds rather than one**, because they want opposite gestures: a gap is answered by an APPEND, a false zero by a SUPERSEDE, since a row exists and is wrong and appending beside it leaves the lie in the record with a correction next to it rather than over it. No `remedy` field is emitted alongside `kind` - it would co-vary with it in every row forever. `outage` also carries `through`, so a run of days is stated rather than rebuilt from a count. **Neither rule carries a number** (G85): silence is measured against the longest gap that source has already shown in this record, and a zero is out of family when the source has never written one for that field before - so a weekly source is not silent after two days, a record that never carried a step count is never asked why it stopped, and a field that legitimately takes zero teaches the engine that by taking it |
